...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    ${CONTAINER_CLI} run  --rm -d --name peer0subcontractor_${CC_NAME}_ccaas \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0manufacturer_${CC_NAME}_ccaas  \
                  ...
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0subcontractor_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer0.manufacturer and peer0.subcontractor
infoln "Installing chaincode on peer0.manufacturer..."
installChaincode 1
infoln "Install chaincode on peer0.subcontractor..."
installChaincode 2
...
## expect manufacturer to have approved and subcontractor not to
checkCommitReadiness 1 "\"ManufacturerMSP\": true" "\"SubcontractorMSP\": false"
checkCommitReadiness 2 "\"ManufacturerMSP\": true" "\"SubcontractorMSP\": false"

## now approve also for subcontractor
approveForMyOrg 2

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness 1 "\"ManufacturerMSP\": true" "\"SubcontractorMSP\": true"
checkCommitReadiness 2 "\"ManufacturerMSP\": true" "\"SubcontractorMSP\": true"
