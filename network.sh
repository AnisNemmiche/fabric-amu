...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-subcontractor.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  infoln "Generating CCP files for Manufacturer and Subcontractor"
...
# call the script to create the channel, join the peers of manufacturer and subcontractor,
...
#   COMPOSE_ORG3_FILES="${COMPOSE_ORG3_BASE_FILES} ${COMPOSE_ORG3_COUCH_FILES} ${COMPOSE_ORG3_CA_FILES}"
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/subcontractor/msp organizations/fabric-ca/subcontractor/tls-cert.pem organizations/fabric-ca/subcontractor/ca-cert.pem organizations/fabric-ca/subcontractor/IssuerPublicKey organizations/fabric-ca/subcontractor/IssuerRevocationPublicKey organizations/fabric-ca/subcontractor/fabric-ca-server.db'
